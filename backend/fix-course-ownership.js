...
async function fixCourseOwnership() {
  try {
    ...
    // Find the trainer@example.com user (the one currently logged in)
    const correctTrainer = await User.findOne({ email: 'trainer@example.com' });
    
    if (!correctTrainer) {
      console.log('❌ trainer@example.com not found');
