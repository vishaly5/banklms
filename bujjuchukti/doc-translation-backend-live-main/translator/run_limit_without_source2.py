...
# https://translationlive.aicte-india.org
...
my_parser.add_argument('--languages', action='append', type=str, required=True)
...
if pages is None:
    ...
    try:
        ...
        if pages < 26:    
            ...
            input_languages = args.languages[0]
            languages = input_languages.split(",")

            # Create a new array to store strings
            dest_lang_array = []
        
            # Assign each element to a string variable in the new array
            for element in languages:
                element = element.strip()
                dest_lang_array.append(element)
            ...
            os.system(("DocumentTranslatorCmd translatedocuments /documents:" +
                    os.path.join(input_dir, filename) + f" /to:{','.join(dest_lang_array)}"))

            root, ext = os.path.splitext(filename)

            for delement in dest_lang_array:
                html_file_name = root+"."+delement+ext
